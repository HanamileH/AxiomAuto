...
class Color:
   ...
   def get_all():
      ...
      try:
         with db.get_cursor(as_dict=True) as cursor:
            cursor.execute("SELECT id AS id, name AS name, hex_code AS hex_code FROM color;")
            rows = cursor.fetchall()
            return rows, ''
      except Exception as e:
         return None, f'server error: {str(e)}'
   ...
   def get_by_id(id):
      ...
      try:
         with db.get_cursor(as_dict=True) as cursor:
            cursor.execute("SELECT id AS id, name AS name, hex_code AS hex_code FROM color WHERE id = %s;", (id,))
            row = cursor.fetchone()
            return row, ''
      except Exception as e:
         return None, f'server error: {str(e)}'
   ...
   def create(name, hex_code):
      ...
      try:
         with db.get_cursor(commit=True) as cursor:
            ...
            # Выполняем вставку
            cursor.execute("INSERT INTO color (name, hex_code) VALUES (%s, %s) RETURNING id", (name, hex_code))
   ...
   def update(id, name, hex_code):
      ...
      try:
         with db.get_cursor(commit=True) as cursor:
            ...
            if name:
               cursor.execute("SELECT id FROM color WHERE name = %s AND id != %s;", (name, id))
               row = cursor.fetchone()
            ...
            # Выполняем обновление
            cursor.execute(f"UPDATE color SET {', '.join(updates)} WHERE id = %s;", params)
            return True, ''
      except Exception as e:
         return False, f'server error: {str(e)}'
...
class Model:
   ...
   @staticmethod
   def create(name, description, price, year, engine_type, engine_power, engine_volume, transmission, brand_id, body_type_id):
      """Создание записи
      Args:
         name (str): Название
      ...
      try:
         with db.get_cursor(commit=True) as cursor:
            ...
            """, (name, description, price, year, engine_type, engine_power, engine_volume, transmission, brand_id, body_type_id))
   ...
   @staticmethod
   def update(id, name, description, price, year, engine_type, engine_power, engine_volume, transmission, brand_id, body_type_id):
      """Обновление записи
      Args:
         id (int): ID записи
         name (str): Название
      ...
      try:
         with db.get_cursor(commit=True) as cursor:
            ...
            cursor.execute(f"UPDATE model SET {', '.join(updates)} WHERE id = %s;", params)
            return True, ''
