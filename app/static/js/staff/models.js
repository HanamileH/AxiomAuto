...
const brandSelect = document.querySelector('#addObjectForm select:nth-child(1)');
const modelNameInput = document.querySelector('#addObjectForm input[placeholder="Название модели"]');
const bodyTypeSelect = document.querySelector('#addObjectForm select:nth-child(3)');
const descriptionInput = document.querySelector('#addObjectForm input[placeholder="Описание"]');
const yearInput = document.querySelector('#addObjectForm input[placeholder="Год"]');
const engineTypeSelect = document.querySelector('#addObjectForm select:nth-child(6)');
const engineVolumeInput = document.querySelector('#addObjectForm input[placeholder="Объём двигателя (л.)"]');
const enginePowerInput = document.querySelector('#addObjectForm input[placeholder="Мощность (л.с.)"]');
const transmissionSelect = document.querySelector('#addObjectForm select:nth-child(9)');
const priceInput = document.querySelector('#addObjectForm input[placeholder="Цена"]');
...
function renderObjectTable() {
   ...
   objects.forEach(model => {
      ...
      row.innerHTML = `
            ...
            <td id="engine-type-${model.id}">${getEngineTypeLabel(model.engine_type)}</td>
            <td id="engine-volume-${model.id}">${model.engine_volume || '-'}</td>
            <td id="engine-power-${model.id}">${model.engine_power}</td>
            <td id="transmission-${model.id}">${getTransmissionLabel(model.transmission)}</td>
            <td id="price-${model.id}">${formatPrice(model.price)}</td>
            <td>
                <button class="btn-outline edit-btn" data-id="${model.id}">Изменить</button>
            </td>
        `;
                ...
                <div class="edit-form-grid">
                    <div class="form-group">
                        ...
                        <select id="edit-brand-${id}" class="edit-input">
                            ...
                            ${window.brands ? window.brands.map(brand => 
                              `<option value="${brand.id}" ${brand.id === object.brand_id ? 'selected' : ''}>${brand.name}</option>`
                            ).join('') : ''}
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="edit-name-${id}" value="${object.model}" class="edit-input" placeholder="Название модели">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="edit-body-type-${id}" class="edit-input">
                            ...
                            ${window.bodyTypes ? window.bodyTypes.map(type => 
                              `<option value="${type.id}" ${type.id === object.body_type_id ? 'selected' : ''}>${type.name}</option>`
                            ).join('') : ''}
                        </select>
                    </div>
                    ...
                    <div class="form-group span-2">
                        ...
                        <textarea id="edit-description-${id}" class="edit-textarea" placeholder="Описание">${object.description}</textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="edit-year-${id}">Год</label>
                        <input type="number" id="edit-year-${id}" value="${object.year || ''}" class="edit-input" placeholder="Год" min="1900" max="2100">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="edit-engine-type-${id}" class="edit-input">
                            ...
                            <option value="petrol" ${object.engine_type === 'petrol' ? 'selected' : ''}>Бензин</option>
                            <option value="diesel" ${object.engine_type === 'diesel' ? 'selected' : ''}>Дизель</option>
                            <option value="hybrid" ${object.engine_type === 'hybrid' ? 'selected' : ''}>Гибрид</option>
                            <option value="electric" ${object.engine_type === 'electric' ? 'selected' : ''}>Электро</option>
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label for="edit-engine-volume-${id}">Объём двигателя (л.)</label>
                        <input type="number" id="edit-engine-volume-${id}" value="${object.engine_volume || ''}" class="edit-input" placeholder="Объём двигателя" step="0.1" min="0">
                    </div>
                    
                    <div class="form-group">
                        <label for="edit-engine-power-${id}">Мощность (л.с.)</label>
                        <input type="number" id="edit-engine-power-${id}" value="${object.engine_power}" class="edit-input" placeholder="Мощность" min="1">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="edit-transmission-${id}" class="edit-input">
                            ...
                            <option value="manual" ${object.transmission === 'manual' ? 'selected' : ''}>Механическая</option>
                            <option value="automatic" ${object.transmission === 'automatic' ? 'selected' : ''}>Автоматическая</option>
                            <option value="variator" ${object.transmission === 'variator' ? 'selected' : ''}>Вариатор</option>
                            <option value="robotic" ${object.transmission === 'robotic' ? 'selected' : ''}>Роботизированная</option>
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label for="edit-price-${id}">Цена</label>
                        <input type="number" id="edit-price-${id}" value="${object.price}" class="edit-input" placeholder="Цена" min="1">
                    </div>
                </div>
...
function validateModelData(data) {
   ...
   if (data.engine_type !== 'electric' && (!data.engine_volume || data.engine_volume <= 0)) {
...
async function saveObject() {
   ...
   const data = {
      ...
      engine_volume: engineTypeSelect.value !== 'electric' ? parseFloat(engineVolumeInput.value) : null,
   ...
   try {
      ...
      if (result.success) {
         ...
         objects.push({
            id: result.id,
            ...data,
            brand: window.brands ? window.brands.find(b => b.id === data.brand_id)?.name || '' : '',
            body_type: window.bodyTypes ? window.bodyTypes.find(t => t.id === data.body_type_id)?.name || '' : ''
         });
